...
class LinkOBSFCTABLES(Task):
    ...
    def execute(self):
        ...
        print('checking if exists:'+ self.config_verif.sqlites_exp_path.replace(self.config_verif.huser,self.config_verif.duser).split('FCTABLE')[0]+'FCTABLE/'+self.config_verif.case)
        #This should be improved because if the link to the duser directory exists but is not filled anymore, it will fail.
        if os.path.exists(self.config_verif.sqlites_exp_path.replace(self.config_verif.huser,self.config_verif.duser).split('FCTABLE')[0]+'FCTABLE/'+self.config_verif.case):
           ...
           if not os.path.exists(os.path.join(self.config_verif.home,f"FCTABLE/"+self.config_verif.case)):
             ...
             print(self.config_verif.sqlites_exp_path.replace(self.config_verif.huser,self.config_verif.duser).split('FCTABLE')[0] +
             'FCTABLE/'+self.config_verif.case)
             print('linked here:')
             print(os.path.join(self.config_verif.home,f"FCTABLE/"+self.config_verif.case))
           ...
           local_exp_sqlites_path=os.path.join(self.config_verif.home,"FCTABLE",self.config_verif.case,self.config_verif.startyyyy,self.config_verif.startmm)
           ecfs_exp_sqlites_path=os.path.join('ec:..',self.config_verif.duser,'deode',self.config_verif.case,'sqlite/*')
           print('making dir for FCTABLES and downloading from ecfs:')
           print(local_exp_sqlites_path)
           print(ecfs_exp_sqlites_path)
           try:
              os.makedirs(local_exp_sqlites_path, exist_ok=True)
           ...
           #os.makedirs(local_exp_sqlites_path,exist_ok=True)
           subprocess.run(["ecp", ecfs_exp_sqlites_path, local_exp_sqlites_path], check=True)           
